...
# ============================================================
def segment_human(image_path, output_mask_path, device, model, target_long=2500):
...
def refine_mask_with_exe(original_image_path, initial_mask_path, refined_mask_path):
    ...
    try:
        ...
        # Rename the result file to our desired name
        if os.path.exists(actual_output_path):
            # Remove existing file if it exists to avoid FileExistsError
            if os.path.exists(refined_mask_path):
                os.remove(refined_mask_path)
            os.rename(actual_output_path, refined_mask_path)
...
# ============================================================
def binarize_mask(mask_path, threshold=100):
    """
...
# ============================================================
def bitwise_threshold_mask(mask_path, threshold=100):
    """
...
def process_refined_mask_with_bitwise(refined_mask_path, output_path, threshold=100):
    """
...
def process_all_images():
    ...
    for fname in tqdm(image_files, desc="Processing images"):
        ...
        if os.path.exists(actual_refined_path):
            ...
            process_refined_mask_with_bitwise(actual_refined_path, bitwise_processed_path, threshold=100)
            ...
            bitwise_processed_path = os.path.join(SKY_MASK_DIR, f"{name}_sky_bitwise_processed.png")
            process_refined_mask_with_bitwise(sky_refined, bitwise_processed_path, threshold=100)
            # Use the bitwise processed mask for final combination
            final_sky_mask = bitwise_processed_path
